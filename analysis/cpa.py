# Python 3 key recovery
# Adapted from code from 2015
#   U. Banerjee, L. Ho, and S. Koppula,
#   “Power-Based Side-Channel Attack for AES Key Extraction on the ATMega328 Microcontroller,”
#   arXiv.org, Mar. 13, 2022. https://arxiv.org/abs/2203.08220.
# https://doi.org/10.48550/arXiv.2203.08220
# https://github.com/skoppula/aes-sidechannel/
...
def run_cpa_fast(traces, plaintext):
    ...
    for byte in range(size_plaintext):
        ...
        state['x.1'] = np.dot(power_hypothesis, np.ones((power_hypothesis.shape[1], power_hypothesis.shape[0])))
        state['y.1'] = np.dot(traces, np.ones((traces.shape[1], traces.shape[0])))
        state['x.x'] = np.dot(np.transpose(power_hypothesis), power_hypothesis)
        state['y.y'] = np.dot(np.transpose(traces), traces)
        state['x.y'] = np.dot(np.transpose(power_hypothesis), traces)
        state['n'] = 9
